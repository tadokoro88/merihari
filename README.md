...
- セッションがアクティブな場合のみ、以下を実行します
  - 指定の時刻範囲内かつグレースケール OFF の時: グレースケールを ON
  - 指定の時刻範囲外かつグレースケール ON の時: グレースケールを OFF
  - 指定の時刻範囲内: 60 秒ごとに通知を表示
- ロック解除 / 復帰 / セッション再アクティブ時は通知をフラッシュし、遅延付きで再チェックします
- 手動でフィルタを切り替えても、60 秒以内に自動で正しい状態へ戻ります
- 設定した時刻情報は以下に保存されます
  `~/.config/merihari/config`
- 詳細仕様は `SPEC.md`、検証観点は `TESTCASES.md` を参照してください
...
- `failed to turn ON/OFF` は、ロック/スリープ状態や復帰直後の不安定なタイミングでも発生することがあります
- `failed to turn ON/OFF` のログは連続 5 回失敗ごとに出力されます（単発の失敗はログに出ない場合があります）
- `failed to turn ON/OFF` が継続して出る場合は、アクセシビリティ権限とショートカット設定も見直してください
- 手動で反映:
```bash
